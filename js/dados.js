...
function obtemDados(collection) {
  ...
  firebase.database().ref(collection).on('value', (snapshot) => {
    ...
    cabecalho.className = 'table-light'
    ...
    snapshot.forEach(item => {
      ...
      novaLinha.classList.add("ricardao")
      ...
      novaLinha.insertCell().innerHTML = `<button class='btn btn-lg btn-outline-danger' onclick=remover('${db}','${id}')>🗑 Excluir</button>
      <button class='btn btn-lg btn-outline-info' onclick=carregaDadosAlteracao('${db}','${id}')>✏️ Editar</button>`

    })
    let rodape = tabela.insertRow()
    rodape.className = 'table-light'
